...
def leo2nlx(cmdrIn, outPath):
    ...
    for vnode in cmdrIn.all_vnodes_iter():
        ...
        nobj.isCloned = vnode.isCloned()
        
        #print "vnd: %s, isCloned = %s" % ( nobj.hdr, nobj.isCloned )
    ...
    for vnode in  _allVnodes():
        ...
        #print "vnode: %s, isCloned = %s" % ( nodeObjx.hdr, nodeObjx.isCloned )
    ...
    for nodex in bffvWalk(hiddenNobj):
        links = [str(nodey.nodeID) for nodey in nodex.parents if nodey.nodeID != nodex.nodeID]
        linkTypes = ['1' for nodey in nodex.parents  if nodey.nodeID != nodex.nodeID]
        
        links = links + [str(nodey.nodeID) for nodey in nodex.children if nodey.nodeID != nodex.nodeID]
    #    linkTypes = linkTypes + ['3' for nodey in nodex.children if nodey.nodeID != nodex.nodeID]
        for nodey in nodex.children:
            if nodey.nodeID != nodex.nodeID:
                if nodey.isCloned:
                    linkTypes = linkTypes + ['2']
                else:
                    linkTypes = linkTypes + ['3']
    ...
    with open(outPath) as f:
        ...
        newText=newText.replace('<network><node', '<network>\n<node')
        newText=newText.replace('/><node', '/>\n<node')
        newText=newText.replace('/></network>', '/>\n</network>')
